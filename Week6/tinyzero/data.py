...
class CurriculumMathDataset(Dataset):
    ...
    def _generate_countdown(self) -> Dict:
        """Generate countdown problem based on current difficulty"""
        ...
        if difficulty == "very_easy":
            ...
            large_nums = random.sample([25, 50, 75], k=2)
            small_nums = random.sample(range(1, 11), k=4)
            numbers = large_nums + small_nums
            target = random.randint(100, 500)
        
        prompt = f"Using {numbers}, make {target}."
    ...
    def _generate_multiplication(self) -> Dict:
        ...
        Difficulty increases automatically as training progresses
        """
        difficulty = self._get_difficulty_level(self.current_step)
        
        if difficulty == "very_easy":
            # Very easy: 2-9 × 2-9 (e.g., 3×4=12, 7×8=56)
            num1 = random.randint(2, 9)
            num2 = random.randint(2, 9)
            
        elif difficulty == "easy":
            # Easy: 2-9 × 10-20 (e.g., 5×12=60, 8×15=120)
            num1 = random.randint(2, 9)
            num2 = random.randint(10, 20)
            
        elif difficulty == "medium":
            # Medium: 10-30 × 10-30 (e.g., 15×20=300, 25×28=700)
            num1 = random.randint(10, 30)
            num2 = random.randint(10, 30)
            
        else:  # hard
            # Hard: 20-99 × 20-99 (e.g., 45×67=3015, 89×92=8188)
